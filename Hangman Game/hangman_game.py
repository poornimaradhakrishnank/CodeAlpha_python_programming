...
# Set up blue theme
root.config(bg="#1C3A9E")

# GUI Layout
top_frame = tk.Frame(root, bg="#1C3A9E")
top_frame.pack(pady=10)

# Word display and attempts left labels
word_label = tk.Label(top_frame, textvariable=displayed_word, font=('Helvetica', 22), bg="#1C3A9E", fg="white")
word_label.pack(pady=10)

attempts_label = tk.Label(top_frame, text="Attempts left: ", font=('Helvetica', 16), bg="#1C3A9E", fg="white")
attempts_label.pack()

attempts_left_label = tk.Label(top_frame, textvariable=attempts_left, font=('Helvetica', 16), bg="#1C3A9E", fg="white")
attempts_left_label.pack()

incorrect_guesses_label = tk.Label(top_frame, text="Incorrect guesses: ", font=('Helvetica', 14), bg="#1C3A9E", fg="white")
incorrect_guesses_label.pack(pady=5)

# Canvas for drawing the hangman
canvas_frame = tk.Frame(root, bg="#1C3A9E")
canvas_frame.pack(side=tk.LEFT, padx=40, pady=20)
canvas = tk.Canvas(canvas_frame, width=300, height=300, bg='#AED6F1')  # Light blue canvas background
canvas.pack()

# Alphabet buttons for guessing letters
buttons_frame = tk.Frame(root, bg="#1C3A9E")
buttons_frame.pack(side=tk.RIGHT, padx=20)

# Create buttons for each letter in a grid with blue background and black text
for letter in 'abcdefghijklmnopqrstuvwxyz':
    btn = tk.Button(buttons_frame, text=letter.upper(), width=4, height=2, bg="#3498DB", fg="black",
                    command=lambda l=letter: guess_letter(l))
    btn.grid(row=(ord(letter) - 97) // 9, column=(ord(letter) - 97) % 9, padx=5, pady=5)

# Reset button to start a new game
reset_button = tk.Button(root, text="Reset Game", command=reset_game, font=('Helvetica', 16), height=2, width=20, bg='#5DADE2', fg='white')
reset_button.pack(pady=20, padx=10, side=tk.BOTTOM)
